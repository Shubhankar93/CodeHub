...
public class VerifyResponseCodeTest {	
	...
	public void callAPI() {
		...
		//Extracting Header
		Response response1 = given().queryParam("key", "qaclick123").headers("content-type", "application/json").body(PayLoad1.PayLoad())
				.when().post("maps/api/place/add/json")
				.then().assertThat().statusCode(200).extract().response();
		String server= response1.getHeader("Server");
